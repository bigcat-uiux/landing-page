<!-- 
  <section class="partners">
    <div class="inner">

      <div class="partners__partner partners__partner--laliga">
        <img src="assets/images/icons/icon-laliga.svg" alt="LaLiga, Official Regional Partner" />
        <div class="partners__partner__info">
          <p class="partners__partner__info__name" data-txt="ftxt1"></p>
          <p class="partners__partner__info__year" data-txt="ftxt1a"></p>
        </div>
      </div>
      <div class="partners__partner partners__partner--bornemouth">
        <img src="assets/images/icons/icon-bournemouth.svg" alt="AFC BOURNEMOUTH">
        <div class="partners__partner__info">
          <p class="partners__partner__info__name" data-txt="ftxt2"></p>
          <p class="partners__partner__info__year" data-txt="ftxt2a"></p>
        </div>
      </div>
      <div class="partners__partner partners__partner--crystalpalace">
        <img src="assets/images/icons/icon-crystalpalace.svg" alt="CRYSTAL PALACE">
        <div class="partners__partner__info">
          <p class="partners__partner__info__name" data-txt="ftxt3"></p>
          <p class="partners__partner__info__year" data-txt="ftxt3a"></p>
        </div>
      </div>
      <div class="partners__partner partners__partner--manchester">
        <img src="assets/images/icons/icon-manchester.svg" alt="MANCHESTER CITY">
        <div class="partners__partner__info">
          <p class="partners__partner__info__name" data-txt="ftxt4"></p>
          <p class="partners__partner__info__year" data-txt="ftxt4a"></p>
        </div>
      </div>
      <div class="partners__partner partners__partner--tottenham">
        <img src="assets/images/icons/icon-tottenham.svg" alt="TOTTENHAM HOTSPURS">
        <div class="partners__partner__info">
          <p class="partners__partner__info__name" data-txt="ftxt5"></p>
          <p class="partners__partner__info__year" data-txt="ftxt5a"></p>
        </div>
      </div>

    </div>
  </section> 
-->

<section class="partners--holder">
  <!-- NEW SPONSORS -->
  <div class="new__sponsors">
    <div class="new--sponsors-title">
      <h1 data-txt="title-sponsors">PROUD SPONSORS OF</h1>
    </div>
    <div class="new__sponsors-inner">
      <div class="sponsor--first-set flex--initial">
        <div class="frame--one frame--flex">
          <a href="https://www.m88.com/la-liga">
            <img src="assets/images/partners/1-la-liga.svg" height="56" width="auto" alt="La Liga">
          </a>
          <p class="sponsor--title strng--txt" data-txt="footer-laliga">Official Regional Partner</p>
          <p class="sponsor--year soft--txt">2020 - 2024</p>
        </div>
        <div class="frame-two frame--flex">
          <a href="https://www.m88.com/partnership">
            <img src="assets/images/partners/2-bwf.svg" alt="BWF">
          </a>
          <p class="sponsor--title strng--txt" data-txt="footer-bwf">Official Betting Partner</p>
          <p class="sponsor--year soft--txt">2022 - 2024</p>
        </div>
        <div class="frame-three frame--flex">
          <a href="https://www.m88.com/partnership">
            <img src="assets/images/partners/3-acmilan.svg" alt="AC MILAN">
          </a>
          <p class="sponsor--title strng--txt" data-txt="footer-ac">Official Casino Partner</p>
          <p class="sponsor--year soft--txt">2022 - 2024</p>
        </div>
      </div>
      <div class="sponsor--second-set flex--secondary">
        <div class="frame-five frame--flex">
          <a href="https://www.m88.com/maria-ozawa">
            <img src="assets/images/partners/5-mariaozawa.svg" height="56" width="auto" alt="MO">
          </a>
          <p class="sponsor--title strng--txt" data-txt="footer-celeb">M88 Celebrity Partner</p>
          <p class="sponsor--year soft--txt">2021 年迄今</p>
        </div>
        <div class="frame-six frame--flex">
          <a href="https://www.m88.com/partnership">
            <img src="assets/images/partners/6-mp.svg" alt="MP">
          </a>
          <p class="sponsor--title strng--txt" data-txt="footer-ambassador">M88 Ambassador</p>
          <p class="sponsor--year soft--txt">2022</p>
        </div>
        <div class="frame-seven frame--flex">
          <a href="https://www.m88.com/partnership">
            <img src="assets/images/partners/7-mansion-ag.svg" height="56" width="auto" alt="Mansin AG">
          </a>
          <p class="sponsor--title strng--txt" data-txt="footer-betting">Official Betting Partner</p>
          <p class="sponsor--year soft--txt">2023</p>
        </div>
      </div>
    </div>
  </div>
  <!-- END OF NEW SPONSORSHIP -->
  <div class="comm__footer">
    <div class="inner__footer">
      <div class="or__partnership-title">
        <h1 data-txt="prev-sponsors">OUR PREVIOUS SPONSORSHIPS</h1>
      </div>
      <div class="or__partnership">
        <div class="or__partnership-sponsor">
          <div class="sponsor__afc">
            <img src="assets/images/partners/1-afc.svg" alt="AFC">
            <div class="txt__tile-sponsor">
              <p>AFC Bournemount</p>
              <p>2016 - 2020</p>
            </div>
          </div>
          <div class="sponsor__crystal">
            <img src="assets/images/partners/2-cp.svg" alt="CRYSTAL">
            <div class="txt__tile-sponsor">
              <p>Crystal Palace</p>
              <p>2015 - 2017</p>
            </div>
          </div>
          <div class="sponsor__manchester">
            <img src="assets/images/partners/4-man.svg" alt="MANCHESTER">
            <div class="txt__tile-sponsor">
              <p>Manchester City</p>
              <p>2011 - 2012</p>
            </div>
          </div>
          <div class="sponsor__tottemham">
            <img src="assets/images/partners/5-th.svg" alt="TOTTEMHAM">
            <div class="txt__tile-sponsor">
              <p>Tottemham Hotspurs</p>
              <p>2006 - 2010</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>